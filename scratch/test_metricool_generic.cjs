const url = "https://mhuxrnxajtiwxauhlhlv.supabase.co/functions/v1/sync-metricool";
...
async function runTest(platform) {
  console.log(`\n=== Testing Haven Generic Sync: ${platform} ===`);
  try {
...
async function main() {
  await runTest('tiktok');
  await runTest('youtube');
}
