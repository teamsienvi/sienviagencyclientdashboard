...
import { Loader2, Linkedin, Check, X, RefreshCw } from "lucide-react";

interface LinkedInOAuthConnectProps {
  clientId?: string;
}

export const LinkedInOAuthConnect = ({ clientId: propClientId }: LinkedInOAuthConnectProps) => {
  ...
  // Fetch connected LinkedIn account for selected client
  ...
  const initOAuthMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to start LinkedIn connection");
    },
  });
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["linkedin-account", clientId] });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Connect a LinkedIn account to sync analytics data
        </CardDescription>
      </CardHeader>
        ...
        {clientId && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Connecting will allow syncing of post analytics and engagement metrics.
            </p>
          </div>
        )}
      </CardContent>
