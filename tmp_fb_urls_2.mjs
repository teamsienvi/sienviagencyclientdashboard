...
async function run() {
  ...
  const { data: posts, error } = await supabase
    .from("social_content")
    .select("platform, title, url, id, published_at")
    .eq("client_id", playiqId)
    .eq("platform", "facebook");
    
  console.log("PlayIQ FB Posts Total:", posts?.length);
  console.log("Posts WITH URL:", posts?.filter(p => p.url).length);
  console.log("Posts WITHOUT URL:", posts?.filter(p => !p.url).length);
