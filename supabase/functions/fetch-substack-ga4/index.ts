...
serve(async (req) => {
  ...
  try {
    ...
    // ── Fetch substack config for client ──
    ...
    if (cfgErr || !config) {
      return new Response(
        JSON.stringify({
          ok: false,
          error: 'Substack GA4 not configured for this client',
    ...
    // ── 1. Summary report ──
    console.log(`Fetching GA4 summary for property ${propertyId} (${config.publication_name})`);
    ...
    const analyticsData = {
      ...
      substackUrl: config.substack_url,
      publicationName: config.publication_name,
    };

    console.log(`GA4 analytics fetched for ${config.publication_name}: ${activeUsers} users, ${sessions} sessions, ${pageViews} pageViews`);

    return new Response(
      JSON.stringify({
        clientId,
        clientName: client?.name || config.publication_name,
    ...
    console.error('Error in fetch-substack-ga4:', error);
